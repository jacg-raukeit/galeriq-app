...
export default function EventDetailScreen() {
    ...
    async (eventId) => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/user/role?event_id=${encodeURIComponent(eventId)}`,
          {
            headers: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/user/event-role?event_id=${encodeURIComponent(
        ...
        const label =
          typeof data === "string" ? data : data?.role ?? data?.name ?? "";
  ...
  const handleUpdate = async () => {
    ...
    if (isOtherType && !eventType.trim()) {
      Alert.alert(
        "Tipo de evento",
        'Escribe el tipo de evento cuando selecciones "Otro".'
      );
      return;
    }
    try {
      ...
      if (coverUri && !String(coverUri).startsWith("http")) {
        ...
        let coverRes = await fetch(
          `${API_URL}/events/${initialEvent.event_id}/cover`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${user.token}` },
            body: coverForm,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          Alert.alert(
            "No encontrado",
            "No se encontró el usuario. Verifica el correo e inténtalo de nuevo."
          );
        } else {
  ...
  const prettyDate = new Date(eventData.event_date).toLocaleDateString(
    "es-ES",
    { day: "2-digit", month: "long", year: "numeric" }
  );
  const prettyTime = new Date(eventData.event_date).toLocaleTimeString(
    "es-ES",
    ...
    <View style={styles.root}>
      ...
      <ScrollView bounces contentContainerStyle={{ paddingBottom: 150 }}>
        ...
        <View style={styles.heroWrapper}>
          <ImageBackground
            source={
              eventData.event_cover ? { uri: eventData.event_cover } : null
            }
            style={styles.hero}
        ...
        <View style={styles.sectionCard}>
          <SectionTitle
            ...
            title="Tu rol en el evento es"
          />
          <Text style={styles.gridValue}>
            {roleLoading ? "Cargando…" : roleLabel || "—"}
          </Text>
        </View>

        {/* Ocultar para invitados (rol 2) */}
        {isOwner && !roleLoading && (
        ...
        <View style={styles.sectionCard}>
          ...
          <Text style={styles.sectionText}>
            {eventData.event_address || "Sin dirección"}
          </Text>
        </View>
      </ScrollView>
          ...
          {roleLoading ? (
            ...
            <>
              ...
              <TabItem
                ...
                label="Acciones"
                active={activeTab === 1 || submenuOpen}
