...
const TOTALS_BY_CATEGORY_URL = (eventId) =>
  `${API_URL}/checklists/event/${eventId}/totals-by-category`;
...
export default function BudgetControlScreen({ route, navigation }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const merged = list.map((it) => {
        ...
        const realSpent =
          Number(it.total_real_price || 0) > 0
  ...
  const spentTotal =
    totals.total_real_price > 0 ? totals.total_real_price : totals.total_budget;
  const pct = useMemo(() => {
